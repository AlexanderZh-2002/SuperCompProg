...
{   
    ...
    for (; (difference > delta); k++){  //метод спуска
        ...
        //#pragma omp parallel for collapse(2) shared(R, A, B, W) private(i,j)
        for (i = 1; i <= n; i++){
            for (j = 1; j <= m; j++){
        ...
        //#pragma omp parallel for collapse(2) reduction(+:loc_rr, loc_Arr) private(i,j)
        for (i = 1; i <= n; i++){
            for (j = 1; j <= m; j++){
                loc_rr += R[i][j] * R[i][j];
        ...
        MPI_Reduce(&loc_rr, &rr, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
        MPI_Reduce(&loc_Arr, &Arr, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
        ...
        //#pragma omp parallel for collapse(2) shared(W, R) reduction(+:loc_scal) private(i,j)
        for (i = 1; i <= n; i++){
            for (j = 1; j <= m; j++){
